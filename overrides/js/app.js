...
function openSpecies(id,from='list'){
  ...
  const cropStyle=t=>{const p=(t.crop||'50% 50%').split(' '),x=parseFloat(p[0])||50,y=parseFloat(p[1])||50;return `--tx:${-(x-12)}%;--ty:${-(y-12)}%`};
  $('#detailContent').innerHTML=`<article class="infographic">
    <section class="info-section info-top" id="detail-animal">
      ...
      <div class="animal-data"><div class="stats-column"><p class="intro">${safe(s.intro||s.fact)}</p><div class="stat"><span class="ico">${svgIcon("population")}</span><div><strong>${safe(s.population)}</strong><b>POPULAÇÃO ESTIMADA</b><small>${safe(s.trend)}</small></div></div><div class="stat"><span class="ico">${svgIcon("status")}</span><div><strong>${safe(s.status)}</strong><b>ESTADO DE CONSERVAÇÃO</b><small>${safe(s.statusLabel)}</small></div></div><div class="stat"><span class="ico">${svgIcon("location")}</span><div><strong>${safe(s.region)}</strong><b>LOCALIZAÇÃO</b><small>${safe(s.place)}</small></div></div></div><div class="animal-visual" style="--animal-pos:${s.photoPosition||'50% 45%'}">${imgTag(s)}</div></div>
      <div class="unique-wrap"><span class="brush">CARACTERÍSTICAS ÚNICAS</span><div class="unique-grid">${traits.map(t=>`<div class="unique-item"><div class="crop"><img src="${t.detailImage||s.image}" alt="${safe(t.title)}" onerror="this.onerror=null;this.src='${s.image}'"></div><div><h4>${safe(t.title)}</h4><p>${safe(t.text)}</p></div></div>`).join('')}</div></div>
    </section>
    <section class="info-section info-middle" id="detail-habitat">
      <div class="habitat-panel"><div class="habitat-image"><img src="${s.habitatImage||s.image}" alt="Habitat de ${safe(s.name)}" onerror="this.onerror=null;this.src='assets/fallback.jpg'"><div class="habitat-title"><span class="brush">HABITAT: ${safe(s.habitat)}</span></div><div class="habitat-icons">${hf.map((x,i)=>`<div><span>${svgIcon(['forest','mountain','climate'][i%3])}</span><b>${safe(x)}</b></div>`).join('')}</div>${s.habitatCredit?`<small class="habitat-credit">${safe(s.habitatCredit)}</small>`:''}</div><div class="habitat-copy">${safe(s.intro)} ${safe(s.habitat)}</div></div>
      <div class="middle-grid"><div class="where-card"><div class="heading"><span class="brush">ONDE VIVE?</span></div><div class="species-range-map" id="speciesRangeMap"></div><div class="range-note"><b>${safe(s.rangeLabel)}</b><br>${safe(s.rangeMapNote||'Área indicativa de ocorrência.')}<small class="micro-source">Fonte cartográfica: ${safe((s.sectionSources||{}).range||'a validar')}</small></div></div><div class="diet-card"><div class="heading"><span class="brush">DIETA</span></div><h3>${safe(s.diet)}</h3><p class="diet-intro">Principais alimentos associados à espécie:</p><div class="diet-icons">${diet.map((d,i)=>`<div><span>${svgIcon((s.dietIconKeys||[])[i]||'leaf')}</span><b>${safe(d)}</b><small>${i===0?'Alimento principal':'Parte da dieta'}</small></div>`).join('')}</div></div></div>
    </section>
    <section class="info-section info-bottom" id="detail-conservation">
      ...
      <div class="closing"><div class="message">${safe(s.conservationMessage||`Preservar ${s.name} é preservar o futuro do habitat que chama de lar.`)}</div><div class="polaroid">${imgTag(s)}<small>${safe(s.name)}</small></div></div>${s.credit?`<div class="info-source-note">${safe(s.credit)}</div>`:''}
    </section>
  </article>`;
  $('#detailFav').onclick=()=>toggleFav(s.id);show('detail');renderDetailSectionNav();setTimeout(()=>renderDetailMap(s),100);
}
function renderDetailMap(s){
  ...
  if(Array.isArray(s.rangePolygon)&&s.rangePolygon.length>2){const poly=L.polygon(s.rangePolygon,{color:'#738d43',weight:2,fillColor:'#8ca755',fillOpacity:.34}).addTo(state.detailMap);state.detailMap.fitBounds(poly.getBounds().pad(.18),{maxZoom:s.detailZoom||7});}
  pts.forEach((p,i)=>L.circleMarker(p,{radius:i===0?7:5,weight:2,color:'#fff',fillColor:i===0?'#e5a12d':'#6c8e45',fillOpacity:1}).addTo(state.detailMap));
  setTimeout(()=>state.detailMap.invalidateSize(),50);
}
function renderDetailSectionNav(){
  const nav=$('#detailSectionNav');if(!nav)return;
  const items=[['detail-animal','Animal'],['detail-habitat','Habitat'],['detail-conservation','Conservação']];
  nav.innerHTML=items.map((x,i)=>`<button class="${i===0?'active':''}" data-target="${x[0]}">${x[1]}</button>`).join('');
  nav.onclick=e=>{const b=e.target.closest('[data-target]');if(!b)return;document.getElementById(b.dataset.target)?.scrollIntoView({behavior:'smooth',block:'start'})};
  const observer=new IntersectionObserver(entries=>{const visible=entries.filter(x=>x.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(!visible)return;nav.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.target===visible.target.id));},{rootMargin:'-22% 0px -58% 0px',threshold:[0,.2,.5]});
  items.forEach(x=>{const el=document.getElementById(x[0]);if(el)observer.observe(el)});
}
boot();
