(()=>{
  ...
  const HERO_SOURCE='assets/generated-v55/sumatran-orangutan/animal.webp';
  const HABITAT=COMMONS+encodeURIComponent('Orang utan sumatra.jpg');
  const DIET=[
    [COMMONS+encodeURIComponent('Fig fruit in the Philippines.jpg'),'FIGOS E FRUTOS','Base energética'],
    [COMMONS+encodeURIComponent('Holly hedge, pruned - geograph.org.uk - 1189638.jpg'),'FOLHAS E REBENTOS','Complemento frequente'],
    [COMMONS+encodeURIComponent('Lichen-Covered Tree Bark Close-Up.jpg'),'CASCA','Recurso em períodos de escassez'],
    [COMMONS+encodeURIComponent('Termites on wood.JPG'),'INSETOS','Complemento proteico']
  ];
  ...
  let heroPromise,queued=false,applying=false;
  const isSpecies=root=>!!root&&((root.querySelector('.info-title h1,h1')?.textContent||'').trim().toUpperCase()===NAME||(root.textContent||'').toUpperCase().includes(NAME));
  async function heroUrl(){
    if(heroPromise)return heroPromise;
    heroPromise=(async()=>{
      const r=await fetch(HERO_SOURCE,{cache:'no-store'}); const ab=await r.arrayBuffer(); const b=new Uint8Array(ab);
      if(b.length>12&&b[0]===82&&b[1]===73&&b[2]===70&&b[3]===70)return URL.createObjectURL(new Blob([b],{type:'image/webp'}));
      const txt=new TextDecoder().decode(b).trim();
      if(/^UklG/.test(txt)){const bin=atob(txt),out=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)out[i]=bin.charCodeAt(i);return URL.createObjectURL(new Blob([out],{type:'image/webp'}));}
      return HERO_SOURCE;
    })();
    return heroPromise;
  }
  async function apply(){
    queued=false;if(applying)return;const root=document.getElementById('detailContent');if(!isSpecies(root))return;
    const hero=root.querySelector('.animal-visual img');if(!hero||hero.dataset.v55Done==='1')return;
    applying=true;
    try{
      const H=await heroUrl();
      hero.src=H;hero.alt='Orangotango-de-Sumatra';hero.dataset.v55Done='1';hero.dataset.v55Zoom='1';hero.classList.add('v40-hero','zoomable-image');hero.style.objectFit='contain';hero.style.objectPosition='center bottom';
      const grid=root.querySelector('.unique-grid');if(grid)grid.innerHTML=traits.map(c=>`<div class="unique-item v40-trait"><div class="crop"><img data-v55-zoom="1" src="${H}" alt="${c[0]}" style="width:100%!important;height:100%!important;object-fit:cover!important;object-position:${c[2]}!important;transform:scale(${c[3]})!important;transform-origin:${c[2]}!important"></div><div><h4>${c[0]}</h4><p>${c[1]}</p></div></div>`).join('');
      const habitat=root.querySelector('.habitat-card img,.habitat-photo img');if(habitat){habitat.src=HABITAT;habitat.alt='Floresta tropical de Sumatra';}
      const card=root.querySelector('.where-card');if(card){let map=card.querySelector('img');const dyn=card.querySelector('.leaflet-container,#speciesRangeMap,.species-range-map');if(dyn&&dyn.tagName!=='IMG'){map=document.createElement('img');dyn.replaceWith(map);}if(map){map.src=MAP;map.alt='Mapa editorial da distribuição do Orangotango-de-Sumatra';map.dataset.v55Zoom='1';map.className='v43-static-map zoomable-image';Object.assign(map.style,{display:'block',width:'100%',height:'auto',maxHeight:'none',objectFit:'contain'});}}
      const dietCard=root.querySelector('.v40-diet-card,.diet-card');if(dietCard)dietCard.innerHTML=`<div class="heading"><span class="brush">DIETA: PRINCIPALMENTE FRUGÍVORA</span></div><p class="v40-diet-intro">Frutos formam a maior parte da dieta quando disponíveis; folhas, rebentos, casca e insetos complementam a alimentação.</p><div class="v40-diet-grid">${DIET.map(d=>`<article><img data-v55-zoom="1" src="${d[0]}" alt="${d[1]}" loading="lazy" referrerpolicy="no-referrer"><b>${d[1]}</b><small>${d[2]}</small></article>`).join('')}</div>`;
      const final=root.querySelector('#detail-conservation .closing .polaroid img');if(final){final.dataset.v55Zoom='1';final.classList.add('zoomable-image');}
    }catch(e){console.error('Orangutan assets',e);}
  ...
  function start(){const root=document.getElementById('detailContent');if(root)new MutationObserver(schedule).observe(root,{childList:true,subtree:true});document.addEventListener('click',e=>{const img=e.target.closest?.('#detailContent img[data-v55-zoom="1"]');if(img&&isSpecies(document.getElementById('detailContent'))){e.preventDefault();e.stopPropagation();openModal(img);return;}setTimeout(schedule,0);},true);schedule();}
