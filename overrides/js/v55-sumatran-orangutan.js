(()=>{
  ...
  function apply(){
    ...
    if(!isSpecies(root)) return;
    const info=root.querySelector('.infographic');
    if(!info || info.dataset.v55Applied==='1') return;
    info.dataset.v55Applied='1';
    info.classList.add('v40-engine','v55-sumatran-orangutan');

    const hero=root.querySelector('.animal-visual img');
    if(hero){
      hero.src=HERO;
      hero.alt='Orangotango-de-Sumatra';
      hero.classList.add('v40-hero','zoomable-image');
    ...
    if(grid){
      grid.innerHTML=traitCfg.map(c=>`<div class="unique-item v40-trait"><div class="crop"><img class="zoomable-image" src="${HERO}" alt="${c[0]}" style="width:100%!important;height:100%!important;object-fit:cover!important;object-position:${c[2]}!important;transform:scale(${c[3]})!important;transform-origin:${c[2]}!important"></div><div><h4>${c[0]}</h4><p>${c[1]}</p></div></div>`).join('');
    }

    const habitat=root.querySelector('.habitat-card img,.habitat-photo img');
    if(habitat){habitat.src=HABITAT;habitat.alt='Floresta tropical de Sumatra, habitat do orangotango-de-Sumatra';}

    const card=root.querySelector('.where-card');
    if(card){
      let map=card.querySelector('img');
      const dyn=card.querySelector('.leaflet-container,#speciesRangeMap,.species-range-map');
      if(dyn&&dyn.tagName!=='IMG'){map=document.createElement('img');dyn.replaceWith(map);}
      if(map){map.src=MAP;map.alt='Mapa editorial da distribuição do Orangotango-de-Sumatra';map.className='v43-static-map zoomable-image';Object.assign(map.style,{display:'block',width:'100%',height:'auto',maxHeight:'none',aspectRatio:'auto',objectFit:'contain',objectPosition:'center'});}
    }
