(()=>{
  ...
  const TRAITS=[
    ['BRAÇOS MUITO LONGOS','Braços poderosos e muito compridos permitem deslocar-se entre as copas sem descer ao solo.','28% 68%','1.55'],
    ['PELAGEM RUIVA','A longa pelagem avermelhada é uma das características mais reconhecíveis da espécie.','28% 48%','2.05'],
    ['FACE EXPRESSIVA','Olhos frontais e grande mobilidade facial acompanham uma elevada inteligência e comunicação visual.','48% 29%','2.45'],
    ['MÃOS PREÊNSEIS','Mãos e pés fortes agarram ramos com precisão e sustentam a vida quase exclusivamente arborícola.','100% 0%','2.15']
  ];
  ...
  async function getHero(){
    ...
    heroPromise=(async()=>{
      const parts=await Promise.all([1,2,3,4,5,6].map(n=>fetch(`${ROOT}animal.part${String(n).padStart(2,'0')}.b64?v=4.3.30`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('orangutan asset '+n);return r.text()})));
      const b64=parts.join('').replace(/\s/g,''),bin=atob(b64),bytes=new Uint8Array(bin.length);
  ...
  function ensureStyles(){
    ...
    const s=document.createElement('style');s.id='v56-orangutan-styles';s.textContent=`
      ...
      #detailContent .v56-diet article:nth-child(n){grid-column:auto!important;transform:none!important}
  ...
  async function apply(){
    ...
    try{
      const HERO=await getHero();if(!isOrangutan(root))return;info.dataset.v56Applied='1';ensureStyles();
